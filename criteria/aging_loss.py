...
class AgingLoss(nn.Module):
    ...
    def forward(self, y_hat, y, target_ages, id_logs, label=None):
        ...
        for i in range(n_samples):
            ...
            if len(id_logs) > i:
                id_logs[i].update({f'input_age': float(input_ages[i]) * 100,
                                   f'output_age': float(output_ages[i]) * 100,
                                   f'target_age': float(target_ages[i]) * 100})
            # otherwise, create a new entry for the sample
            else:
                id_logs.append({f'input_age': float(input_ages[i]) * 100,
                                f'output_age': float(output_ages[i]) * 100,
                                f'target_age': float(target_ages[i]) * 100})
